	...
	function camelGet(target, prop) {
		...
		if (subEntries.length > 0) {
			...
			const error = new Error(
				`Missing environment variable! "${propConstantCase}" does not exist, nor do any vars beginning with "${propPrefix}".`
			);
...
export function envProxy(namespace = false, env = process.env) {
	const proxy = autoProxy(Object.entries(env));
	return namespace ? proxy[namespace] : proxy;
}

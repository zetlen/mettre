...
export class PutSyncer extends EventEmitter {
	...
	async release({ filePath }) {
		...
		try {
			...
			if (!transfer) {
				this.logger.warn({ filePath }, `Transfer not found for file "${filePath}". Will delete file directly`);
				await this.client.Files.Delete([file.id], { skipTrash: true });
			} else {
	...
	async #removeTransferIfDone(transfer, filePath) {
		...
		await this.client.post('/transfers/remove', {
      data: {
        transfer_ids: transfer.id.toString()
      },
    })
	}
	...
	async submit(dropped) {
		await this.#updateTransfers();
		const { data: torrentData } = dropped;
		let transfer = this.currentState.transfers.byHash.get(

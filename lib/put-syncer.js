...
export class PutSyncer extends EventEmitter {
	...
	async start() {
		...
		for (const transfer of transfers) {
			this.logger.trace(
				{ transfer },
				"%s transfer %s found %s% complete on startup!",
				transfer.status,
				transfer.name,
	...
	async release({ filePath }) {
		...
		this.logger.warn('release("%s"): skipping for now!', filePath);
		return;
		this.logger.info(
	 ...
	 */
	async download(transfer, isRetry = false) {
		...
		if (!this.#getTransferByHash(transfer.hash)) {
			...
			setTimeout(() => this.download(transfer, true), 2000);
			return;
		}
		await this.#updateFiles();
		this.#emitTransferDownloads(transfer);
